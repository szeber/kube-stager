...
type RedisConfigDeleteHandler struct {
	Client client.Client
}

func (r *RedisConfigDeleteHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	logger := log.FromContext(ctx)
	logger.Info("Validating deletion of redis config " + req.Namespace + "/" + req.Name)
	...
	err := r.Client.List(
		...
		client.MatchingLabels{labels.RedisEnvironmentsPrefix + req.Name: "true"},
	)
	if nil != err {
	...
	err = r.Client.List(
		...
		&serviceList,
		client.InNamespace(req.Namespace),
		client.MatchingFields{indexes.DefaultRedisEnvironment: req.Name},
	)
	if nil != err {
		return admission.Errored(http.StatusInternalServerError, err)
	}
