...
// TestMongoConfigDeleteHandler_SitesUsingEnvironment verifies that a MongoConfig
// cannot be deleted when StagingSites carry the label
// mongo.environments.operator.kube-stager.io/<name>=true.
// The handler denies at the label check before reaching the MatchingFields query.
func TestMongoConfigDeleteHandler_SitesUsingEnvironment(t *testing.T) {
	const ns = "test-ns"
	const envName = "my-mongo"

	site := testutil.NewTestStagingSite("mysite", ns, nil)
	site.Labels = map[string]string{
		labels.MongoEnvironmentsPrefix + envName: "true",
	}

	handler := &MongoConfigDeleteHandler{
	...
	if resp.Allowed {
		t.Error("expected Denied when sites use the mongo environment, got Allowed")
	}
...
func TestMongoConfigDeleteHandler_MultipleSitesUsingEnvironment(t *testing.T) {
	const ns = "test-ns"
	const envName = "my-mongo"
	...
	if resp.Allowed {
		t.Error("expected Denied when multiple sites use the mongo environment, got Allowed")
	}
}

// TestMongoConfigDeleteHandler_NoReferences tests that the label-only check passes
...
func TestMongoConfigDeleteHandler_NoReferences(t *testing.T) {
