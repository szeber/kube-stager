...
func (r *MysqlDatabaseReconciler) doReconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.ImportHandler.IsObjectBeingImported(importer.TYPE_MYSQL_DATABASE, req.NamespacedName) {
		logger.Info("Skipping database as import is in progress")
	...
	if !db.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		db.ObjectMeta.Finalizers = helpers.RemoveFromSlice(db.ObjectMeta.Finalizers, helpers.MysqlFinalizerName)
	...
	if !helpers.SliceContains(db.ObjectMeta.Finalizers, helpers.MysqlFinalizerName) {
