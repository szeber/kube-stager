...
//+kubebuilder:rbac:groups=importer.operator.kube-stager.io,resources=importexportdata,verbs=get;create;update;patch;delete
//+kubebuilder:rbac:groups=importer.operator.kube-stager.io,resources=importexportdata/status,verbs=get;update;patch
...
func (r *StagingSiteReconciler) doReconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.ImportHandler.IsObjectBeingImported(importer.TYPE_STAGING_SITE, req.NamespacedName) {
		logger.Info("Skipping site as import is in progress")
	...
	if !helpers.SliceContains(site.Finalizers, helpers.SiteFinalizerName) {
...
func (r *StagingSiteReconciler) handleDelete(ctx context.Context, logger logr.Logger, site *sitev1.StagingSite) (
	...
	site.Finalizers = helpers.RemoveFromSlice(site.Finalizers, helpers.SiteFinalizerName)
...
func (r *StagingSiteReconciler) SaveStatusUpdatesIfObjectChanged(
	...
	importErr := r.updateOrCreateImportExportData(ctx, site)
...
func (r *StagingSiteReconciler) updateOrCreateImportExportData(ctx context.Context, site *sitev1.StagingSite) error {
	importExportData := &importerv1.ImportExportData{}
	dataExists := true

	if err := r.Get(ctx, client.ObjectKey{Namespace: site.Namespace, Name: site.Name}, importExportData); err != nil {
		if client.IgnoreNotFound(err) != nil {
			return err
		}
	...
	importExportData.Spec.SiteSpec = site.Spec
	importExportData.Spec.SiteStatus = site.Status
	importExportData.Status.IsImportComplete = true

	r.addSiteAsOwnerReferenceIfNotAdded(importExportData, site)

	importedObject := &importer.ImportedObject{
		ImportType: importer.TYPE_IMPORT_EXPORT_DATA,
		ObjectKey:  types.NamespacedName{},
		StartedAt:  r.Clock.Now(),
	}

	r.ImportHandler.Add(importedObject)

	if dataExists {
		if err := r.Update(ctx, importExportData); nil != err {
			return err
		}
	} else {
		if err := r.Create(ctx, importExportData); nil != err {
			return err
		}
	}

	err := r.Status().Update(ctx, importExportData)

	r.ImportHandler.Delete(importedObject)

	return err
}

func (r *StagingSiteReconciler) addSiteAsOwnerReferenceIfNotAdded(importExportData *importerv1.ImportExportData, site *sitev1.StagingSite) {
	boolTrue := true
	...
	for _, i := range importExportData.OwnerReferences {
		if i.APIVersion == site.APIVersion && i.Kind == site.Kind && i.Name == site.Name && i.UID == site.UID {
			return
		}
	}

	importExportData.ObjectMeta.OwnerReferences = append(importExportData.ObjectMeta.OwnerReferences, ownerReference)
}
